...
def notify_if_above_threshold(
    ...
    """Send `score`'s listing to every not-yet-notified channel, once match_score
    clears the threshold. Dedup is per listing+channel — a listing that already
    triggered a Discord alert won't re-alert there even from a later re-score."""
    threshold = threshold if threshold is not None else settings.notification_score_threshold
    channels = channels if channels is not None else tuple(available_channels())
    ...
    for channel in channels:
        ...
        try:
            get_notifier(channel)(listing, score)
            status = "sent"
        except Exception:
            status = "failed"
